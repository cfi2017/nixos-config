...
}:
{
  ...
  config = lib.mkIf config.cfi2017.gpg.enable {
    ...
    cfi2017.core.zfs = lib.mkIf (config.cfi2017.isLinux && config.cfi2017.persistence.enable) {
      homeDataLinks = [
        {
          directory = ".gnupg";
          mode = "0700";
    ...
    environment.systemPackages = lib.optionals config.cfi2017.isLinux [ pkgs.gnupg ];
    ...
    home-manager.users.${config.cfi2017.user.name} = {
      ...
      services.gpg-agent = {
        ...
        extraConfig =
          lib.optionalString config.cfi2017.isDarwin ''
            pinentry-program /opt/homebrew/bin/pinentry-mac
          ''
          + lib.optionalString (config.cfi2017.isLinux && !(config.cfi2017.graphical.enable or false)) ''
